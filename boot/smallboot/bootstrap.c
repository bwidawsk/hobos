...
extern void main
(int size, unsigned char drive_letter, struct partition *partition_entry) __attribute__((noreturn));

/* We used to have a bss and clear it, but we don't anymore */
extern unsigned char _bstart, _bend, _btext, _edata;

/*
 * We enter here in 32 bit protected mode and 0x6a00  (sp=0x7a00 - 0x1000 for IVT)
 * of stack space.  Interrupts are turned off.  For now we just want to load the
 * remaining sectors of our 1.5 stage bootloader.  We make at least the following
 * assumptions which should be abstracted later
 *	1. sector size of the load medium is 512b
 *	2. x86 legacy interrupts can read the
 *	load medium
 */
